...
ax.set_title(f"Induction cliff ({MODEL}): per-position loss on a repeated random sequence", fontweight="bold")
...
ax.set_title(f"Induction score by (layer, head) — {MODEL}", fontweight="bold")
...
ax.axvline(cliff, color="#9FAAAD", ls="--", lw=1)
ax.set_title(f"Break it ({MODEL}): ablating the induction heads kills in-context learning", fontweight="bold")
